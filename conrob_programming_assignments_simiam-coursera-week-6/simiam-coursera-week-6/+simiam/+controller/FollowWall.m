...
%             obj.p = simiam.util.Plotter();
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            if(strcmp(inputs.direction,'right'))
                ...
                ir_rw= ir_distances(3:5);                               
                ir_rw(2)= ir_rw(2) * 0.999999999;
                [val, ind] = sort(ir_rw); 
               ...
               direction=1;
                ...
                %p_2= ir_distances_wf(:,ind(1));
                %p_1= ir_distances_wf(:,ind(2));
            end
            ...
            scale_factor= val(1)+val(2);
            
            %disp('scale factor = ' + scale_factor);
                
            disp(val);
            disp(direction);
            if(strcmp(inputs.direction,'right'))
                if ((val(1)<0.18) && (val(2)< 0.18))
                    u_fw = u_fw_tp*direction +  u_fw_pp * scale_factor * -130 ;
                    disp('close');
                elseif ((val(1)>0.14) && (val(2)> 0.14))
                    disp('far');
                    %u_fw = u_fw_tp .* direction +  u_fw_pp;
                    u_fw = u_fw_tp .* direction +  u_fw_pp * scale_factor * 10;
                else
                    disp('normal');
                    u_fw = u_fw_tp .* direction +  u_fw_pp;
                end
            else
                if ((val(1)<0.15) || (val(2)< 0.15))
                    u_fw = u_fw_tp*direction +  u_fw_pp * scale_factor * -70 ;
                    disp('close');
                elseif ((val(1)>0.2) && (val(2)> 0.2))
                    disp('far');
                    %u_fw = u_fw_tp .* direction +  u_fw_pp;
                    u_fw = u_fw_tp .* direction +  u_fw_pp * scale_factor * 10;
                else
